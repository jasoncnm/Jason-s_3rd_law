...
bool MoveAction(IVec2 actionDir)
{
    ...
    if (mother->attach)
    {
        ...
        {
            if (pushResult.blockedEntity->type == ENTITY_TYPE_PIT ||
...
bool SplitAction(Entity * player, IVec2 bounceDir)
{
    ...
    {
        player = MergeSlimes(player, clone);
        return false;
    }
...
void GameUpdateAndRender(GameState * gameStateIn, Memory * gameMemoryIn)
{
    ...
    // NOTE: Render Step
    {
        ...
        if (!animationPlaying)
        {
            ...
            {
                DrawSprite(texture, gameState->leftArrow.sprite, gameState->leftArrow.topLeftPos, gameState->leftArrow.tileSize);
            }
        
            // Right
            if (gameState->rightArrow.show)
            {
                DrawSprite(texture, gameState->rightArrow.sprite, gameState->rightArrow.topLeftPos, gameState->rightArrow.tileSize);
            }
        
            // Up
            if (gameState->upArrow.show)
            {
                DrawSprite(texture, gameState->upArrow.sprite, gameState->upArrow.topLeftPos, gameState->upArrow.tileSize);
            }
        
            // Down
            if (gameState->downArrow.show)
            {
                DrawSprite(texture, gameState->downArrow.sprite, gameState->downArrow.topLeftPos, gameState->downArrow.tileSize);
            }
        }
