...
#include "../vendor/raygui/examples/styles/style_cyber.h"       // raygui style: cyber
#include "../vendor/raygui/examples/styles/style_jungle.h"      // raygui style: jungle
#include "../vendor/raygui/examples/styles/style_lavanda.h"     // raygui style: lavanda
#include "../vendor/raygui/examples/styles/style_dark.h"        // raygui style: dark
#include "../vendor/raygui/examples/styles/style_bluish.h"      // raygui style: bluish
#include "../vendor/raygui/examples/styles/style_terminal.h"    // raygui style: terminal
 ...
 void test()
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        // Help button
        if (GuiButton(Rectangle{ contentRect.x, contentRect.y + contentRect.height + scrollOffset.y, contentRect.width, 1.5f*fontSize }, GuiIconText(showHelp? ICON_EYE_ON : ICON_EYE_OFF, "Curve controls help"))) showHelp = !showHelp;
        ...
        // Load default curves
        if (GuiButton(Rectangle{ contentRect.x, contentRect.y+contentRect.height+scrollOffset.y, contentRect.width, 1.5f*fontSize }, "Load default"))
        {
            LoadCurveDefaults(curves);
            animationTime = 4.0f;
            time = 0.0f;
        ...
        GuiComboBox(Rectangle{contentRect.x, contentRect.y+contentRect.height+scrollOffset.y, contentRect.width, 1.5f*fontSize }, "default;Jungle;Lavanda;Dark;Bluish;Cyber;Terminal", &visualStyleActive);
        contentRect.height += 1.5f*fontSize + margin;
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            // Draw curve control
            Rectangle curveRect = Rectangle{ contentRect.x, contentRect.y+contentRect.height + scrollOffset.y, contentRect.width, fontSize*12.0f };
